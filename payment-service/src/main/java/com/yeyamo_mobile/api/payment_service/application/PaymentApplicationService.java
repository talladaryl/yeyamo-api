...
import static com.yeyamo_mobile.api.payment_service.application.PaymentCommands.*;import java.util.*;import java.math.*;import org.springframework.stereotype.Service;import org.springframework.transaction.annotation.Transactional;
...
@Service @Transactional public class PaymentApplicationService{
 ...
 public RefundEntity refund(Refund command){
  ...
  if(result.outcome()==Outcome.SUCCEEDED){refund.succeeded(required(result.providerOperationId(),"providerRefundId"));refunds.save(refund);payment.refunded(alreadyRefunded.add(refund.getAmount()).compareTo(payment.getAmount())>=0);payments.save(payment);refunded(payment,refund,command.correlationId());}
  else if(result.outcome()==Outcome.FAILED){refund.failed(reason(result));payment.refundFailed(reason(result),alreadyRefunded.signum()>0);refunds.save(refund);payments.save(payment);refundFailed(payment,refund,reason(result),command.correlationId());}
 ...
 public void providerRefundSucceeded(String providerRefundId,String correlation){RefundEntity refund=refunds.findByProviderRefundId(providerRefundId).orElseThrow(()->notFound("provider refund"));if(refund.getStatus()==RefundStatus.SUCCEEDED)return;PaymentEntity payment=locked(refund.getPayment().getBookingId());refund.succeeded(providerRefundId);refunds.save(refund);completeRefund(payment);payments.save(payment);refunded(payment,refund,correlation);}
 public void providerRefundFailed(String providerRefundId,String reason,String correlation){RefundEntity refund=refunds.findByProviderRefundId(providerRefundId).orElseThrow(()->notFound("provider refund"));if(refund.getStatus()==RefundStatus.FAILED)return;PaymentEntity payment=locked(refund.getPayment().getBookingId());refund.failed(reason);payment.refundFailed(reason,refundedAmount(payment).signum()>0);refunds.save(refund);payments.save(payment);refundFailed(payment,refund,reason,correlation);}
