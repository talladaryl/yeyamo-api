...
@Component public class WebhookSignatureVerifier{
 ...
 public WebhookSignatureVerifier(@Value("${payment.webhook.secret}")String secret,@Value("${payment.webhook.tolerance-seconds:300}")long tolerance){if(secret==null||secret.length()<24)throw new IllegalArgumentException("PAYMENT_WEBHOOK_SECRET must contain at least 24 characters");this.secret=secret.getBytes(StandardCharsets.UTF_8);this.tolerance=tolerance;}
