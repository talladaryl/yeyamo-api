package com.yeyamo_mobile.api.booking_service.infrastructure.security;
...
public class SecurityConfig {
    ...
    static final class JwtRolesConverter implements Converter<Jwt, AbstractAuthenticationToken> {
        public AbstractAuthenticationToken convert(Jwt jwt) {
            ...
            var many = jwt.getClaimAsStringList("roles");
            if (many != null) roles.addAll(many);
            var one = jwt.getClaimAsString("role");
            if (one != null && !one.isBlank()) roles.add(one);
            Collection<GrantedAuthority> a = roles.stream()
