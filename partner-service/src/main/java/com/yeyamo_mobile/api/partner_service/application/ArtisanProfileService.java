...
public class ArtisanProfileService {
    ...
    @Transactional(readOnly=true) public Page<PublicResponse> search(String country,String area,String city,UUID specialty,Boolean custom,Boolean shipping,String search,Pageable pageable){Specification<ArtisanProfileEntity>spec=(root,q,cb)->{q.distinct(true);List<Predicate>filters=new ArrayList<>();filters.add(cb.equal(root.get("verificationStatus"),ArtisanProfileEntity.VerificationStatus.VERIFIED));if(country!=null&&!country.isBlank())filters.add(cb.equal(root.get("countryCode"),new CountryReference(country).countryCode()));if(area!=null&&!area.isBlank())filters.add(cb.equal(root.get("adminLevel1Id"),area));if(city!=null&&!city.isBlank())filters.add(cb.equal(root.get("cityId"),city));if(specialty!=null)filters.add(cb.equal(root.join("specialties").get("id"),specialty));if(custom!=null)filters.add(cb.equal(root.get("acceptsCustomOrders"),custom));if(shipping!=null)filters.add(cb.equal(root.get("internationalShipping"),shipping));if(search!=null&&!search.isBlank()){String like="%"+search.toLowerCase(Locale.ROOT).trim()+"%";filters.add(cb.or(cb.like(cb.lower(root.get("displayName")),like),cb.like(cb.lower(root.get("craftDescription")),like)));}return cb.and(filters.toArray(Predicate[]::new));};return profiles.findAll(spec,pageable).map(PublicResponse::from);}
    @Transactional(readOnly=true) public PublicResponse publicProfile(UUID id){return PublicResponse.from(profiles.findById(id).filter(value->value.getVerificationStatus()==ArtisanProfileEntity.VerificationStatus.VERIFIED).orElseThrow(this::missing));}
