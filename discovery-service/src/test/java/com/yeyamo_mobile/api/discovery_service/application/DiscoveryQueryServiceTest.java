...
class DiscoveryQueryServiceTest {
 @Test void usesCacheWithoutSearching(){var port=mock(DiscoverySearchPort.class);var cache=mock(DiscoveryCachePort.class);var criteria=criteria();var cached=new DiscoveryPage(0,2,false,List.of(doc("1")),Instant.now());when(cache.get(criteria)).thenReturn(Optional.of(cached));assertSame(cached,new DiscoveryQueryService(port,cache,mock(AdInjectionService.class)).search(criteria));verifyNoInteractions(port);}
 @Test void computesHasNextAndCachesPage(){var port=mock(DiscoverySearchPort.class);var cache=mock(DiscoveryCachePort.class);var criteria=criteria();when(cache.get(criteria)).thenReturn(Optional.empty());when(port.search(criteria,3)).thenReturn(List.of(doc("1"),doc("2"),doc("3")));var page=new DiscoveryQueryService(port,cache,mock(AdInjectionService.class)).search(criteria);assertTrue(page.hasNext());assertEquals(2,page.items().size());verify(cache).put(eq(criteria),same(page));}
