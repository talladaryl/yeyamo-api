...
public record PlaceSuggestionResponse(
        ...
        UUID administrativeAreaId, UUID cityId, UUID localityId, List<PlaceSuggestionMediaResponse> media
) {
    public static PlaceSuggestionResponse from(PlaceSuggestion suggestion) {
        return new PlaceSuggestionResponse(suggestion.getId(), suggestion.getSubmitterUserId(), suggestion.getName(),
                ...
                suggestion.getAdministrativeAreaId(), suggestion.getCountryCityId(), suggestion.getLocalityId(),
                suggestion.getMedia().stream().map(PlaceSuggestionMediaResponse::from).toList());
    }
}
