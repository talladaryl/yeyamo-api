...
public class PostApplicationService{
 ...
 @Transactional public Post createDraft(String authorId,PostCommand c,String correlationId){references.requirePublic(c.referenceType(),c.referenceId());validateGeography(c);Post p=Post.draft(authorId,c.caption(),c.visibility(),c.catalogAssetId(),c.mediaIds(),c.hashtags());p.reference(c.referenceType(),c.referenceId());p.setGeography(geography(c));
  p=repository.save(p);outbox.append("content.post.created",p,correlationId,authorId);return p;}
 @Transactional public Post updateDraft(UUID id,String actorId,boolean admin,PostCommand c,String correlationId){references.requirePublic(c.referenceType(),c.referenceId());validateGeography(c);Post p=owned(id,actorId,admin);p.updateDraft(c.caption(),c.visibility(),c.catalogAssetId(),c.mediaIds(),c.hashtags());p.reference(c.referenceType(),c.referenceId());p.setGeography(geography(c));
