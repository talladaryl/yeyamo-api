...
public class JpaPostRepositoryAdapter implements PostRepository{
 ...
 private PostEntity entity(Post p){PostEntity e=new PostEntity();e.setId(p.getId());e.setAuthorId(p.getAuthorId());e.setCaption(p.getCaption());e.setStatus(p.getStatus());e.setVisibility(p.getVisibility());e.setCatalogAssetId(p.getCatalogAssetId());e.setReferenceType(p.getReferenceType());e.setReferenceId(p.getReferenceId());
  e.setMediaIds(new ArrayList<>(p.getMediaIds()));e.setHashtags(new LinkedHashSet<>(p.getHashtags()));e.setCreatedAt(p.getCreatedAt());e.setUpdatedAt(p.getUpdatedAt());e.setPublishedAt(p.getPublishedAt());e.setArchivedAt(p.getArchivedAt());e.setDeletedAt(p.getDeletedAt());e.setVersion(p.getVersion());return e;}
 private Post domain(PostEntity e){Post p=new Post();p.setId(e.getId());p.setAuthorId(e.getAuthorId());p.setCaption(e.getCaption());p.setStatus(e.getStatus());p.setVisibility(e.getVisibility());p.setCatalogAssetId(e.getCatalogAssetId());p.setReferenceType(e.getReferenceType());p.setReferenceId(e.getReferenceId());
  p.setMediaIds(e.getMediaIds());p.setHashtags(e.getHashtags());p.setCreatedAt(e.getCreatedAt());p.setUpdatedAt(e.getUpdatedAt());p.setPublishedAt(e.getPublishedAt());p.setArchivedAt(e.getArchivedAt());p.setDeletedAt(e.getDeletedAt());p.setVersion(e.getVersion());return p;}
}
